...
def watch_tesla(TESLA_URL, specifyModel):
    ...
    # Comment out the headless option to visually see the browser
    ...
    logPrint(f'Finished loading Tesla URL.')

    # Print the webpage content
    print(driver.page_source)  # This will print the HTML content of the page to the console

    try:
        time.sleep(5)  # Increase sleep time to allow page to load
        logPrint(f'Scanning webpage...')
        results_container = WebDriverWait(driver, 30).until(
            EC.presence_of_element_located((By.CLASS_NAME, "results-container"))
        )
        logPrint(f'Finished scanning webpage.')
